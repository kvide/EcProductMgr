...
$inline = 0;
$resultpage = $returnid;
$thetemplate = product_utils::get_template($params, 'searchformtemplate', 'EcProductMgr::Search');
...
$fields = array();
{
    ...
    if (count($tmp))
    {
        foreach ($tmp as $obj)
        {
            ...
            {
                if ($obj->type == 'text' || $obj->type == 'textarea' || $obj->type == 'textbox')
                {
                    $obj->type = 'text';
                }
                if ($obj->type == 'dropdown')
                {
                    $obj->options = \xt_array::hash_prepend($obj->options, - 1, $this->Lang('any'));
                }
                $fields[$obj->name] = $obj;
            }
        }
...
{
    $searchfield = trim($params['searchfield']); // this param is deprecated
}
...
if (! empty($searchfield))
{
    ...
    {
        $tpl->assign('searchprops', $searchfields);
    }
}
else
{
    if (count($fields))
    {
        $tpl->assign('searchprops', $fields);
    }
}

$tpl->assign('cd_origpage', $returnid);
$tpl->assign('formstart', $this->XTCreateFormStart($id, 'do_search', $resultpage, $params, $inline));
