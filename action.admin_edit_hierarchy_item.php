...
$hier_id = \xt_param::get_int($params, 'hierarchy_id');
if ($hier_id > 0)
{
    $sql = 'SELECT * FROM ' . cms_db_prefix() . 'module_ec_prodmgr_hierarchy WHERE id = ?';
...
if (\xt_param::exists($params, 'submit'))
{
    ...
    {
        $old_rec = $hier_rec;
        $hier_rec['parent_id'] = \xt_param::get_int($params, 'parent_id', $hier_rec['parent_id']);
        $hier_rec['name'] = \xt_param::get_string($params, 'name', $hier_rec['name']);
        $hier_rec['description'] = \xt_utils::get_param($params, 'description', $hier_rec['description']);
        $hier_rec['extra1'] = \xt_param::get_string($params, 'extra1', $hier_rec['extra1']);
        $hier_rec['extra2'] = \xt_param::get_string($params, 'extra2', $hier_rec['extra2']);

        if (! $hier_rec['name'])
        {
            throw new \InvalidArgumentException($this->Lang('err_noname'));
        }

        if ($hier_rec['id'] && $hier_rec['parent_id'] != $old_rec['parent_id'])
        {
            // parent change on update.
            $query = 'SELECT id FROM ' . cms_db_prefix() . 'module_ec_prodmgr_hierarchy WHERE parent_id = ? AND name = ?';
            $tmp = $db->GetOne($query, [
                $hier_rec['parent_id'],
                $hier_rec['name']
            ]);
            if ($tmp)
            {
                throw new \InvalidArgumentException($this->Lang('error_nameused'));
            }
        }
        ...
        if ($hier_rec['id'] > 0)
        {
            ...
            if (\xt_param::exists($params, 'deleteimg') && $hier_rec['image'])
            {
            ...
            if ($old_rec['parent_id'] != $hier_rec['parent_id'] && $hier_rec['item_order'] > - 1)
            {
                ...
                if ($hier_rec['parent_id'] > 0
                    && product_utils::hierarchy_isparentof($hier_rec['parent_id'], $hier_rec['id']))
                {
                ...
                $query = 'UPDATE ' . cms_db_prefix() . 'module_ec_prodmgr_hieriarchy SET item_order = item_order - 1
                ...
                $query = 'SELECT COALESCE(MAX(item_order), 0) + 1 FROM ' . cms_db_prefix() . 'module_ec_prodmgr_hierarchy
                          WHERE parent_id = ?';
                $hier_rec['item_order'] = (int) $db->GetOne($query, [$hier_rec['parent_id']]);
            }

            $query = 'UPDATE ' . cms_db_prefix() . 'module_ec_prodmgr_hierarchy
        ...
        {
            // it's an insert
            $query = 'SELECT MAX(item_order) + 1 FROM ' . cms_db_prefix() . 'module_ec_prodmgr_hierarchy'
                        . ' WHERE parent_id = ?';
            $item_order = (int) $db->GetOne($query, [$hier_rec['parent_id']]);
            if (! $item_order)
            {
                $item_order = 1;
            }

            $query = 'INSERT INTO ' . cms_db_prefix() . 'module_ec_prodmgr_hierarchy
        ...
        if ($have_upload)
        {
            ...
            if (! is_dir($destdir))
            {
                throw new \RuntimeException($this->Lang('err_mkdir', $destdir));
            }
            ...
            if (! $res)
            {
                throw new \RuntimeException($this->GetUploadErrormessage($err));
            }
            if ($res)
            {
                $hier_rec['image'] = $res;
                $sql = 'UPDATE ' . cms_db_prefix() . 'module_ec_prodmgr_hierarchy SET image = ? WHERE id = ?';
...
echo $this->ProcessTemplate('admin_edit_hierarchy_item.tpl');
