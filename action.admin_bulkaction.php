...
if (isset($params['multiselect']) && is_array($params['multiselect'])
    && count($params['multiselect']) && isset($params['bulkaction']))
{
    try
    {
        ...
        $bulk_modify_product_status = function ($product_list, $status)
        {
            ...
            if ($rs->totalrows == 0)
            {
                throw new \LogicException('Internal error: Products did not match in query');
            }
            while (! $rs->EOF)
            {
        ...
        switch ($params['bulkaction'])
        {
            ...
            case 'setdraft':
                $bulk_modify_product_status($params['multiselect'], Product::STATUS_DRAFT);
                break;

            case 'setdisabled':
                $bulk_modify_product_status($params['multiselect'], Product::STATUS_DISABLED);
                break;

            case 'setpublished':
                $bulk_modify_product_status($params['multiselect'], Product::STATUS_PUBLISHED);
                break;
