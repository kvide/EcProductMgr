...
elseif (isset($params['submit']))
{
    ...
    $query = 'SELECT * FROM ' . cms_db_prefix() . 'module_ec_prodmgr_hierarchy ORDER by hierarchy';
    $dbr = $db->GetArray($query);
    $dbr = \xt_array::to_hash($dbr, 'id');

    // 3. Update the database
    $query = 'UPDATE ' . cms_db_prefix() . 'module_ec_prodmgr_hierarchy SET parent_id = ?, item_order = ?,'
                . ' hierarchy = ?, long_name = ? WHERE id = ?';
    foreach ($data as $rec)
    {
...
// give it to smarty
//
$smarty->assign('formstart', $this->XTCreateFormStart($id, 'admin_reorder_hierarchy'));
