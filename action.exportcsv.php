...
function to_flat_array($product, $map)
{
    ...
    if ($_cats == - 1)
    {
        $_cats = product_utils::get_categories(TRUE);
    }
    static $_defs = - 1;
    if ($_defs == - 1)
    {
        $_defs = product_utils::get_fielddefs(true, true, true);
    }
        ...
        switch ($key)
        {
            ...
            default:
                ...
                elseif (startswith($key, 'FIELD:'))
                {
                    ...
                    $def = $_defs[$name];
                    $fld_id = $def->id;
                    if (isset($product->field_vals) && count($product->field_vals)
                        && array_key_exists($fld_id, $product->field_vals))
                    {
...
$smarty->assign('formstart', $this->XTCreateFormStart($id, 'exportcsv'));
