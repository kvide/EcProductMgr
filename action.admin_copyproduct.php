...
$get_unique_sku = function ($input)
{
    ...
    if (($p = strpos($input, '_copy')) !== FALSE)
    {
        if ($p !== FALSE)
        {
            $idx = (int) substr($input, $p + strlen('_copy'));
        }
        $idx = max(2, $idx + 1);
    }
...
try
{
    ...
    if (! $product)
    {
        throw new \LogicException($this->Lang('error_productnotfound'));
    }
    ...
    if ($product2->alias)
    {
        $product2->set_alias(product_ops::generate_alias($product2->name));
    }
    if ($product2->sku)
    {
        $product2->set_sku($get_unique_sku($product2->sku));
    }
