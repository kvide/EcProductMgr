...
if (! isset($params['parent']) && $active_hierarchy > 0)
{
    $params['parent'] = $active_hierarchy;
}
    ...
    function products_byhierarchy_postprocess(&$data, $params, $summarypage, $hierpage, $inline, $id)
    {
        if (is_array($data))
        {
            ...
            for ($i = 0; $i < count($data); $i ++)
            {
                ...
                $rec['down_url'] = $module->create_url($id, 'hierarchy', $hierpage, $parms, $inline);

                $parms = $params;
                $parms['hierarchyid'] = $rec['id'];
                if (isset($parms['summarypage']))
                {
                    unset($parms['summarypage']);
                }
                if ($rec['count'] > 0)
                {
                ...
                {
                    products_byhierarchy_postprocess($rec['children'], $params, $summarypage, $hierpage, $inline, $id);
                }
            }
        }
    }
...
$thetemplate = product_utils::get_template($params, 'hierarchytemplate', 'EcProductMgr::Hierarchy View');
...
elseif (isset($params['hierarchy']))
{
    ...
    $tmp2 = implode(',', $tmp2);
    $query = 'SELECT id FROM ' . cms_db_prefix() . 'module_ec_prodmgr_hierarchy WHERE name IN (' . $tmp2 . ')';
...
if (! count($parents))
{
    return; // nothing found to start with
}
...
if (count($nodes))
{
    ...
    $nodes = implode(',', $nodes);
    $query = 'SELECT ph.*, count(pr.id) AS count FROM ' . cms_db_prefix() . 'module_ec_prodmgr_hierarchy ph
              LEFT OUTER JOIN ' . cms_db_prefix() . 'module_ec_prodmgr_prodtohier pth
                ON ph.id = pth.hierarchy_id
              LEFT OUTER JOIN ' . cms_db_prefix() . 'module_ec_prodmgr pr
...
if (count($parents) == 1)
{
    ...
    if ($hierdata)
    {
        ...
        $hierdata['parent_url'] = $this->create_url($id, 'hierarchy', $returnid,
            array('parent' => $hierdata['parent_id']));
        $tpl->assign('active_hierarchy', $parents[0]);
        if ($inline == 0)
        {
            $hierdata['canonical'] = $this->create_url($id, 'hierarchy', $returnid,
