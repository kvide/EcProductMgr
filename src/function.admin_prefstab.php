...
$smarty->assign('input_status', $this->CreateInputDropdown($id, 'status', $statuses, - 1,
                                                            $this->GetPreference('default_status', 'published')));
$smarty->assign('input_taxable', // $this->CreateInputHidden($id,'taxable',0).
$this->CreateInputCheckbox($id, 'taxable', 1, $this->GetPreference('default_taxable', 1)));
...
$smarty->assign('input_summarysortorder', $this->CreateInputDropdown($id, 'sortorder', $sortorders, - 1, $sortorder));

if (! class_exists('\EcommerceExt\ecomm'))
{
    $smarty->assign('prompt_currencysymbol', $this->Lang('prompt_currencysymbol'));
    $smarty->assign('input_currencysymbol', $this->CreateInputText($id, 'currencysymbol',
                                                        $this->GetPreference('products_currencysymbol')));

    $smarty->assign('prompt_weightunits', $this->Lang('prompt_weightunits'));
    $smarty->assign('input_weightunits', $this->CreateInputText($id, 'weightunits',
    ...
    $smarty->assign('input_lengthunits', $this->CreateInputDropdown($id, 'lengthunits', $opts, - 1,
                    $this->GetPreference('products_lengthunits')));
}

$smarty->assign('input_allowed_imagetypes', $this->CreateInputText($id, 'allowed_imagetypes',
                    $this->GetPreference('allowed_imagetypes'), 50, 255));

$smarty->assign('input_allowed_filetypes', $this->CreateInputText($id, 'allowed_filetypes',
...
$smarty->assign('input_summarysorting', $this->CreateInputDropdown($id, 'sortby', $sortings, - 1, $sortby));

$smarty->assign('input_deleteproductfiles', $this->CreateInputYesNoDropdown($id, 'deleteproductfiles',
                    $this->GetPreference('deleteproductfiles')));

$smarty->assign('input_usehierpathurls', $this->CreateInputYesNoDropdown($id, 'usehierpathurls',
                    $this->GetPreference('usehierpathurls', 0)));

$smarty->assign('input_use_detailpage_for_search', $this->CreateInputYesNoDropdown($id, 'use_detailpage_for_search',
                    $this->GetPreference('use_detailpage_for_search', 0)));

$smarty->assign('input_hierpage', $contentops->CreateHierarchyDropdown('',
                    $this->GetPreference('hierpage'), $id . 'hierpage'));
...
$smarty->assign('input_prodnotfoundpage', $contentops->CreateHierarchyDropdown('',
                    $this->GetPreference('prodnotfoundpage', - 1), $id . 'prodnotfoundpage'));
