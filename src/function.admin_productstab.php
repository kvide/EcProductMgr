...
//
if (isset($_SESSION['products_sel_pagenum']))
{
    $pagenumber = (int) $_SESSION['products_sel_pagenum'];
}
...
if ($pagenumber > 1)
{
    $_SESSION['products_sel_pagenum'] = $pagenumber;
}
...
if (isset($params['submit']))
{
    ...
    {
        $filter['sku'] = ''; // one, or the other.. but not both.
    }
    ...
    {
        $filter['custom_fields'] = $params['custom_fields'];
    }
    ...
    $filter['excludecats'] = (int) \xt_utils::get_param($params, 'input_excludecats');
...
$all_fields = \xt_array::to_hash($all_fields, 'name');

$filterinuse = ($filter == $bare_filter) ? FALSE : TRUE;
$tpl->assign('formstart', $this->XTCreateFormStart($id, 'defaultadmin'));
...
$tpl->assign('input_hierarchy',
                $this->CreateHierarchyDropdown($id, 'input_hierarchy', $filter['hierarchy']));
$tpl->assign('input_children',
                $this->CreateInputYesNoDropdown($id, 'input_children', $filter['children']));
$tpl->assign('input_sortby',
                $this->CreateInputDropdown($id, 'input_sortby', $sortitems, - 1, $filter['sortby']));
$tpl->assign('input_sortorder',
                $this->CreateInputDropdown($id, 'input_sortorder', $sortorders, - 1, $filter['sortorder']));
$tpl->assign('input_pagelimit',
                $this->CreateInputDropdown($id, 'input_pagelimit', $pagelimits, - 1, $filter['pagelimit']));
$tpl->assign('input_excludecats',
                $this->CreateInputYesNoDropdown($id, 'input_excludecats', $filter['excludecats']));
...
$fields = array();
$fields[] = 'DISTINCT p.*';
$prefix1 = "SELECT <FIELDS> FROM " . cms_db_prefix() . "module_ec_prodmgr p ";
$prefix2 = "SELECT count(p.id) AS count FROM " . cms_db_prefix() . 'module_ec_prodmgr p ';
$where = array();
$joins = array();
$qparms = array();
...
elseif ($filter['sku'])
{
    ...
    $tmp = str_replace('%%', '%', $tmp);
    $joins[] = 'LEFT JOIN ' . cms_db_prefix() . 'module_ec_prodmgr_attribs PA ON p.id = PA.product_id';
...
if (count($filter['custom_fields']) && count($fields_viewable))
{
    for ($j = 0; $j < count($filter['custom_fields']); $j ++)
    {
        ...
        $joins[] = 'LEFT OUTER JOIN ' . cms_db_prefix()
                    . "module_ec_prodmgr_fieldvals Fv{$j} ON Fv{$j}.product_id = p.id AND Fv{$j}.fielddef_id = " . $fid;
    }
}

if (! empty($filter['hierarchy']))
{
    $tquery = 'SELECT long_name FROM ' . cms_db_prefix() . 'module_ec_prodmgr_hierarchy WHERE id = ?';
    $long_name = $db->GetOne($tquery, [(int) $filter['hierarchy']]);

    $joins[] = 'LEFT JOIN ' . cms_db_prefix() . 'module_ec_prodmgr_prodtohier ph ON ph.product_id = p.id';
    $joins[] = 'LEFT JOIN ' . cms_db_prefix() . 'module_ec_prodmgr_hierarchy h ON h.id = ph.hierarchy_id';
...
{
    $joins[] = 'LEFT OUTER JOIN ' . cms_db_prefix() . 'module_ec_prodmgr_product_categories pc ON p.id = pc.product_id';
...
if (startswith($filter['sortby'], 'F::'))
{
    ...
    $joins[] = 'LEFT JOIN ' . cms_db_prefix() . 'module_ec_prodmgr_fieldvals fv ON fv.product_id = p.id';
...
if (($totalcount % $filter['pagelimit']) != 0)
{
    $pagecount ++;
}
...
// Begin the output
//
$tpl->assign('totalproducts', $db->GetOne('SELECT COUNT(id) FROM ' . cms_db_prefix() . 'module_ec_prodmgr'));
...
if ($pagenumber > 1)
{
    ...
    $tpl->assign('firstpage_url', $this->create_url($id, 'defaultadmin', '', $parms));
    $parms = array('pagenumber' => $pagenumber - 1);
    $tpl->assign('prevpage_url', $this->create_url($id, 'defaultadmin', '', $parms));
}
if ($pagenumber < $pagecount)
{
    $parms = array('pagenumber' => $pagenumber + 1);
    $tpl->assign('nextpage_url', $this->create_url($id, 'defaultadmin', '', $parms));
    $parms = array('pagenumber' => $pagecount);
    $tpl->assign('lastpage_url', $this->create_url($id, 'defaultadmin', '', $parms));
}
$entryarray = array();
...
while ($dbresult && $row = $dbresult->FetchRow())
{
    ...
    $row['edit_url'] = $this->create_url($id, 'editproduct', $returnid, [
        'compid' => $row['id']
    ]);
    $row['editlink'] = $this->CreateLink($id, 'editproduct', $returnid, $theme->DisplayImage('icons/system/edit.gif',
        $this->Lang('edit'), '', '', 'systemicon'), array('compid' => $row['id']));
    $row['copylink'] = $this->CreateLink($id, 'admin_copyproduct', $returnid, $theme->DisplayImage('icons/system/copy.gif',
        $this->Lang('copy_product'), '', '', 'systemicon'), array('compid' => $row['id']));
    $row['deletelink'] = $this->CreateLink($id, 'deleteproduct', $returnid, $theme->DisplayImage('icons/system/delete.gif',
        $this->Lang('delete'), '', '', 'systemicon'), array('compid' => $row['id']), $this->Lang('areyousure_deleteproduct'));
    $entryarray[] = $row;
}
