...
$query = 'SELECT * FROM ' . cms_db_prefix() . 'module_ec_prodmgr_hierarchy WHERE id = ?';
...
$query = 'UPDATE ' . cms_db_prefix() . 'module_ec_prodmgr_hierarchy SET item_order = item_order - 1
...
$query = 'UPDATE ' . cms_db_prefix() . 'module_ec_prodmgr_hierarchy SET parent_id = -1 WHERE parent_id = ?';
$dbr = $db->Execute($query, array($hierarchy_id));

// Now remove it
$query = 'DELETE FROM ' . cms_db_prefix() . 'module_ec_prodmgr_hierarchy WHERE id = ?';
$db->Execute($query, array($hierarchy_id));

// And now update any products to have no category
// if they have this hierarchy_id
$query = 'DELETE FROM ' . cms_db_prefix() . 'module_ec_prodmgr_prodtohier WHERE hierarchy_id = ?';
$db->Execute($query, array($hierarchy_id));
