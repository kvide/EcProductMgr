...
{ // TODO: Why this scope?
  ...
  // and filter them for the ones that we can search on.
    $tmp = product_utils::get_fielddefs(false, true, FALSE);
    if (count($tmp))
    {
        foreach ($tmp as $fdid => $obj)
        {
...
$thetemplate = product_utils::get_template($params, 'summarytemplate', 'EcProductMgr::List View');
$destpage = $returnid;
$entryarray = '';
$npages = 0;
$inline = 0;
...
try
{
    ...
    if (isset($params['cd_prodname']) && $params['cd_prodname'])
    {
        $products_query['productnamelike'] = trim($params['cd_prodname']);
    }
    if (isset($params['cd_proddesc']) && $params['cd_proddesc'])
    {
        $products_query['desclike'] = trim($params['cd_proddesc']);
    }
    if (isset($params['cd_allany']))
    {
        $products_query['useor'] = $params['cd_allany'];
    }
    ...
    elseif (isset($params['cd_prodprice']) && $params['cd_prodprice'] != - 1)
    {
    ...
    foreach ($tfields as $one)
    {
        ...
        if (isset($params['cd_prodvalue'][$one]) && $params['cd_prodvalue'][$one] != '-1'
    ...
    {
        $results->detailtemplate = $params['detailtemplate'];
    }
    $entryarray = array();
    while (! $results->EOF)
    {
