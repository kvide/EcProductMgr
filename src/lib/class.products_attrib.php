...
final class products_attrib implements \ArrayAccess
{
    ...
    public function offsetSet($key, $value)
    {
        ...
        switch ($key)
        {
            ...
            case 'product_id':
                ...
                if ($value < 1)
                {
                    throw new \CmsException('product id cannot be negative or zero for products_attrib object');
                }
                break;
            ...
            case 'qoh':
                ...
                if ($value < 0)
                {
                    throw new \CmsException('QOH cannot be negative for products_attrib object');
                }
                break;
    ...
    private function _insert()
    {
        ...
        if ($this->_data['iorder'] < 1)
        {
            ...
            $query = 'SELECT MAX(COALESCE(iorder,0))+1 FROM ' . cms_db_prefix()
                        . 'module_ec_prodmgr_attribs WHERE product_id = ?';
            $this->_data['iorder'] = $db->GetOne($query, array(
                $this->_data['product_id']
            ));
        }

        $data = $this->_data;
        $query = 'INSERT INTO ' . cms_db_prefix() . 'module_ec_prodmgr_attribs
    ...
    private function _update()
    {
        ...
        $db = cmsms()->GetDb();
        $query = 'UPDATE ' . cms_db_prefix() . 'module_ec_prodmgr_attribs
    ...
    public static function load_from_form($params)
    {
        ...
        if ((count($params['sku']) != count($params['text'])) || (count($params['sku']) != count($params['adjustment']))
            || (count($params['sku']) != count($params['qoh'])) || (count($params['sku']) != count($params['notes'])))
        {
    ...
    {
        $db = cmsms()->GetDb();
        $query = 'DELETE FROM ' . cms_db_prefix() . 'module_ec_prodmgr_attribs
              WHERE product_id = ?';
