...
class ProductStorage
{
    ...
    public function __construct(\EcProductMgr $mod, $db)
    {
        if (self::$_inst > 0)
        {
    ...
    private function _insert_custom_fields($product_id, $product_arr)
    {
        if (isset($product_arr['field_vals']) && count($product_arr['field_vals']))
        {
            ...
            $sql = 'INSERT INTO ' . self::product_fieldvals_table_name() . ' (product_id, fielddef_id, value,'
                        . ' create_date, modified_date) VALUES (?,?,?,?,?)';
            foreach ($product_arr['field_vals'] as $fielddef_id => $val)
            {
                if ($defs[$fielddef_id]->type == 'dimensions' || $defs[$fielddef_id]->type == 'subscription'
    ...
    private function _insert_category_records($product_id, $product_arr)
    {
        ...
        {
            $db = $this->_db;
            $sql = 'INSERT INTO ' . self::product_categories_table_name() . ' (product_id, category_id, create_date,'
    ...
    private function _insert_attrib_records($product_id, $product_arr)
    {
        ...
        if (isset($product_arr['attribs']) && count($product_arr['attribs']))
        {
            $db = $this->_db;
            $iorder = 1;
            $sql = 'INSERT INTO ' . self::product_attribs_table_name() . ' (product_id, text, adjustment, sku, qoh,'
                        . ' notes, iorder) VALUES (?,?,?,?,?,?,?)';
    ...
    private function _insert_product(Product $product)
    {
        ...
        $adjust_parms = [
            'id' => null,
            'create_date' => $this->db_datestring(),
            'modified_date' => $this->db_datestring()
        ];
        $sql = 'INSERT INTO ' . self::product_table_name() . ' (product_name, price, details, taxable, digital, status,'
                    . ' weight, sku, alias, url, owner, extra, create_date, modified_date)'
                    . ' VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
        $i_arr = [
            $product->name,
            $product->price,
            ...
            $product->owner,
            serialize($arr['extra']),
            $adjust_parms['create_date'],
            $adjust_parms['modified_date']
        ];
        $dbr = $db->Execute($sql, $i_arr);
        if (! $dbr)
        {
            throw new \xt_sql_error('SYSTEM ERROR: Problem inserting product: ' . $db->ErrorMsg());
        }
        ...
        \CMSMS\HookManager::do_hook('ProductMgr::AfterInsertProduct', $product);
    ...
    private function _delete_connected_data($product_id, $delete_files = false)
    {
        ...
        if ($delete_files)
        {
            ...
            \xt_dir::recursive_remove_directory($dir);
        }
    ...
    private function _update_product(Product $product)
    {
        ...
        \CMSMS\HookManager::do_hook('ProductMgr::BeforeUpdateProduct', $product);
        $arr = $product->to_array();

        $sql = 'UPDATE ' . self::product_table_name() . ' SET product_name = ?, price = ?, details = ?, taxable = ?,'
                    . ' digital = ?, status = ?, weight = ?, sku = ?, alias = ?, url = ?, owner = ?, extra = ?,'
        ...
        \CMSMS\HookManager::do_hook('ProductMgr::AfterUpdateProduct', $product);
    }
    ...
    public function check_attribs(Product $product)
    {
        ...
        foreach ($attribs as $attrib)
        {
            if (empty($product->sku))
            {
                ...
                {
                    throw new \RuntimeException($this->_mod->Lang('error_attrib_skurequired'));
                }
            }
            else
            {
    ...
    private function _build_from_row(array $row)
    {
        ...
        // get categories
        $sql = 'SELECT category_id FROM ' . self::product_categories_table_name() . ' WHERE product_id = ?'
        ...
        // get hierarchies
        $sql = 'SELECT hierarchy_id FROM ' . self::product_hierarchies_table_name() . ' WHERE product_id = ?'
        ...
        // get field values
        $sql = 'SELECT fielddef_id, value FROM ' . self::product_fieldvals_table_name() . ' WHERE product_id = ?'
                    . ' ORDER BY fielddef_id';
    ...
    {
        \CMSMS\HookManager::do_hook('ProductMgr::BeforeDeleteProduct', $product_id);
        ...
        $query = 'DELETE FROM ' . cms_db_prefix() . 'module_ec_prodmgr_prodtohier WHERE product_id = ?';
        $db->Execute($query, [$product_id]);
        ...
        $query = "DELETE FROM " . cms_db_prefix() . "module_ec_prodmgr WHERE id = ?";
        $db->Execute($query, [$product_id]);
        ...
        \CMSMS\HookManager::do_hook('ProductMgr::AfterDeleteProduct', ['product_id' => $product_id]);
    }
    ...
    {
        $uid = (int) $uid;
        $sql = 'SELECT COUNT(id) FROM ' . self::product_table_name() . ' WHERE owner = ?';
        $db = $this->_db;
    ...
    {
        return CMS_DB_PREFIX . 'module_ec_prodmgr';
    }

    public static function product_categories_table_name()
    {
        return CMS_DB_PREFIX . 'module_ec_prodmgr_product_categories';
    }

    public static function product_hierarchies_table_name()
    {
        return CMS_DB_PREFIX . 'module_ec_prodmgr_prodtohier';
    }

    public static function product_fieldvals_table_name()
    {
        return CMS_DB_PREFIX . 'module_ec_prodmgr_fieldvals';
    }

    public static function product_attribs_table_name()
    {
        return CMS_DB_PREFIX . 'module_ec_prodmgr_attribs';
    }
