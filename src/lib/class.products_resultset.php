...
final class products_resultset extends \CMSMSExt\query\resultset
{
     ...
     * build the sql query to extract the product data.
     */
    protected function _query()
    {
        ...
        $query = 'SELECT SQL_CALC_FOUND_ROWS C.*, PH.hierarchy_id FROM ' . CMS_DB_PREFIX . 'module_ec_prodmgr C';
        $where = array();
        $qparms = array();
        $joins = array();

        $joins[] = CMS_DB_PREFIX . 'module_ec_prodmgr_prodtohier PH ON PH.product_id = C.id';
        ...
        {
            $subquery = 'SELECT DISTINCT product_id FROM ' . CMS_DB_PREFIX . 'module_ec_prodmgr_attribs WHERE sku = ?';
            $subparms = array(
                $obj['optsku']
            );
        ...
        {
            $subquery = 'SELECT DISTINCT id FROM ' . CMS_DB_PREFIX . 'module_ec_prodmgr_hierarchy h WHERE ';
            $subparms = array();
        ...
        else if (isset($obj['hierarchyid']))
        {
            ...
            else
            {
                ...
                $joins[] = CMS_DB_PREFIX . 'module_ec_prodmgr_hierarchy H ON H.id = PH.hierarchy_id';
        ...
        if (isset($obj['category']) && $obj['category'] != '')
        {
            ...
            if (count($icatids))
            {
                $joins[] = CMS_DB_PREFIX . 'module_ec_prodmgr_product_categories cc ON cc.product_id = C.id';
                $where[] = 'cc.category_id IN (' . implode(',', $icatids) . ')';
            }
            else
            {
                debug_buffer('products_resultset - category parameter provided to query (' . $obj['category']
                                . '), but no matching categories found');
            }
        }
        else if (isset($obj['categoryid']) && $obj['categoryid'] != '')
        {
            $joins[] = CMS_DB_PREFIX . 'module_ec_prodmgr_product_categories pc ON pc.product_id = C.id';
        ...
        foreach ($obj->get_field_exprs() as $fieldid => $fieldval)
        {
            if ($fieldval == '::null::')
            {
                ...
                $joins[] = sprintf(CMS_DB_PREFIX . "module_ec_prodmgr_fieldvals FV{$suffix} ON C.id = FV{$suffix}."
                                    . "product_id AND FV{$suffix}.fielddef_id = %d", $fieldid);
            ...
            else if (strpos($fieldval, ':') !== FALSE)
            {
                ...
                elseif ($max)
                {
                    $joins[] = CMS_DB_PREFIX . "module_ec_prodmgr_fieldvals FV{$suffix} ON C.id = FV{$suffix}."
                                . "product_id AND FV{$suffix}.fielddef_id = ?";
                    $where[] = "(CAST(FV{$suffix}.value AS DECIMAL(12,3)) <= ?)";
                    $max = floatval($max);
                    $qparms[] = $max;
                }
        ...
        if (startswith($sortby, 'f:'))
        {
            ...
            {
                $joins[] = CMS_DB_PREFIX . 'module_ec_prodmgr_fieldvals FVS ON C.id = FVS.product_id AND'
                            . ' FVS.fielddef_id = ' . (int) $fielddefs[$sortby]->id;
                $sortby = 'FVS.value';
            }
            else
            {
    ...
    private function _preload_categories($product_ids)
    {
        ...
        $db = cmsms()->GetDb();
        $query = 'SELECT product_id,category_id FROM ' . CMS_DB_PREFIX . 'module_ec_prodmgr_product_categories
    ...
    private function _preload_hierarchies($product_ids)
    {
        ...
        $sql = 'SELECT product_id, hierarchy_id FROM ' . CMS_DB_PREFIX . 'module_ec_prodmgr_prodtohier
    ...
    private function _preload_fields($product_ids)
    {
        ...
        $db = cmsms()->GetDb();
        $query = 'SELECT product_id,fielddef_id,value FROM ' . CMS_DB_PREFIX . 'module_ec_prodmgr_fieldvals
    ...
    private function _preload_attributes($product_ids)
    {
        ...
        $db = cmsms()->GetDb();
        $query = 'SELECT * FROM ' . CMS_DB_PREFIX . 'module_ec_prodmgr_attribs
    ...
    public function get_product($as_array = FALSE)
    {
        ...
        if (is_array($this->_attribute_cache) && isset($this->_attribute_cache[$product_id])
            && is_array($this->_attribute_cache[$product_id]) && count($this->_attribute_cache[$product_id]))
        {
    ...
    public function get_product_for_display()
    {
        ...
        $obj = new DisplayableProduct($product, $arr);
