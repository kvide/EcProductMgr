...
final class productsCsvImporter
{
    ...
    private function _setLine($tmp)
    {
        $this->_line = $tmp;
        $this->_linenum ++;
        $this->_record = \xt_array::smart_explode($this->_line, $this->_delim);
    ...
    private function _getFieldId($name)
    {
        ...
        {
            $db = cmsms()->GetDb();
            $query = 'SELECT id, name, type FROM ' . cms_db_prefix() . 'module_ec_prodmgr_fielddefs';
            $tmp = $db->GetArray($query);
            if (is_array($tmp))
            {
                $this->_field_map = \xt_array::to_hash($tmp, 'name');
            }
        }
    ...
    {
        $db = cmsms()->GetDb();
        $query = 'SELECT value FROM ' . cms_db_prefix() . 'module_ec_prodmgr_fieldvals WHERE product_id = ?'
                    . ' AND fielddef_id = ?';
        ...
        if ($tmp)
        {
            if ($val != '')
            {
                $query = 'UPDATE ' . cms_db_prefix() . 'module_ec_prodmgr_fieldvals SET value = ? WHERE product_id = ?'
                            . ' AND fielddef_id = ?';
            ...
            {
                $query = 'DELETE FROM ' . cms_db_prefix() . 'module_ec_prodmgr_fieldvals WHERE product_id = ?'
                            . ' AND fielddef_id = ?';
                $dbr = $db->Execute($query, array(
                    $val,
                    $product_id,
        ...
        else
        {
            ...
            if ($val != '')
            {
                $query = 'INSERT INTO ' . cms_db_prefix() . 'module_ec_prodmgr_fieldvals (value, product_id,'
                            . ' fielddef_id) VALUES (?,?,?)';
                $dbr = $db->Execute($query, array(
                    $val,
                    $product_id,
    ...
    private function _getCategoryId($name)
    {
        ...
        {
            $db = cmsms()->GetDb();
            $query = 'SELECT id, name FROM ' . cms_db_prefix() . 'module_ec_prodmgr_categories';
            $tmp = $db->GetArray($query);
            if (is_array($tmp))
            {
                $this->_category_map = \xt_array::to_hash($tmp, 'name');
            }
        }
    ...
    private function _getHierarchyCache()
    {
        ...
        $db = cmsms()->GetDb();
        $query = 'SELECT id,long_name FROM ' . cms_db_prefix() . 'module_ec_prodmgr_hierarchy';
        $tmp = $db->GetArray($query);
        if (! is_array($tmp))
        {
            return;
    ...
    {
        $db = cmsms()->GetDb();
        $query = 'SELECT hierarchy_id FROM ' . cms_db_prefix() . 'module_ec_prodmgr_prodtohier WHERE product_id = ?';
        $tmp = $db->GetOne($query, array(
            $product_id
        ));
        if ($tmp)
        {
            $query = 'UPDATE ' . cms_db_prefix() . 'module_ec_prodmgr_prodtohier SET hierarchy_id = ?'
                        . ' WHERE product_id = ?';
            $dbr = $db->Execute($query, array(
                $hier_id,
                $product_id
        ...
        {
            $query = 'INSERT INTO ' . cms_db_prefix() . 'module_ec_prodmgr_prodtohier (product_id, hierarchy_id)'
                        . ' VALUES (?,?)';
            $dbr = $db->Execute($query, array(
                $product_id,
                $hier_id
    ...
    private function _getHierarchyId($hier)
    {
        $search_hierarchy_id = function ($hier)
        {
            ...
            if (! $this->_hierarchy_map)
            {
                return;
            }
            if (isset($this->_hierarchy_map[$hier]))
            {
        ...
        if (! $the_hid && $this->_getPolicyValue('create_hierarchy'))
        {
            ...
            $fquery = 'SELECT id FROM ' . cms_db_prefix() . 'module_ec_prodmgr_hierarchy WHERE name = ?'
                        . ' AND parent_id = ?';
            $iquery = 'INSERT INTO ' . cms_db_prefix() . 'module_ec_prodmgr_hierarchy (name, parent_id) VALUES (?,?)';
            foreach ($path as $one)
            {
                $name = trim($one);
                $name = trim($name, '"\'');
                ...
                if (! $t_id)
                {
                    ...
                    \EcProductMgr\product_utils::update_hierarchy_positions();
                }
                $parent = $t_id;
            }
            $the_hid = $t_id;
    ...
    private function _hasField($name)
    {
        ...
        if (! is_array($this->_defs_byname))
        {
            return FALSE;
        }
        if (! isset($this->_defs_byname[$name]))
        {
            return FALSE;
        }
        $idx = $this->_defs_byname[$name];
        if ($idx >= count($this->_record))
        {
            return FALSE;
        }
    ...
    private function _getDate($name, $dflt = null)
    {
    ...
    {
        if (! is_array($this->_defs_byindex))
        {
            return FALSE;
        }
        $res = array();
    ...
    private function _handleFirstLine()
    {
        ...
        if ($this->_getPolicyValue('create_categories'))
        {
            ...
            if (is_array($cats))
            {
                $fquery = 'SELECT id FROM ' . cms_db_prefix() . 'module_ec_prodmgr_categories WHERE name = ?';
                $iquery = 'INSERT INTO ' . cms_db_prefix() . "module_ec_prodmgr_categories (name, create_date,"
        ...
        if ($this->_getPolicyValue('create_fields'))
        {
            ...
            if (is_array($fields))
            {
                ...
                $fquery = 'SELECT id FROM ' . cms_db_prefix() . 'module_ec_prodmgr_fielddefs WHERE name = ?';
                $oquery = 'SELECT max(item_order) + 1 FROM ' . cms_db_prefix() . 'module_ec_prodmgr_fielddefs';
                $iquery = 'INSERT INTO ' . cms_db_prefix() . "module_ec_prodmgr_fielddefs
    ...
    private function _handleProductLine()
    {
        ...
        $fquery = 'SELECT id FROM ' . cms_db_prefix() . 'module_ec_prodmgr WHERE product_name = ?';
        $t_id = $db->GetOne($fquery, array($name));
        $is_update = FALSE;
        if ($t_id)
        {
        ...
        else
        {
            ...
            $product_id = $t_id;
            $uquery = 'UPDATE ' . cms_db_prefix() . 'module_ec_prodmgr
                 SET product_name = ?, details = ?, price = ?, modified_date = ?, taxable = ?, status = ?,'
                    . ' weight = ?, sku = ? WHERE id = ?';
            $dbr = $db->Execute($uquery, array(
                $this->_getField('#name'),
                $this->_getField('#details', ''),
                $this->_getCurrency($this->_getField('#price', 0)),
                $this->_getDate('#modified_date', $tnow),
        ...
        if ($hier)
        {
            ...
            if ($hid)
            {
                $this->_setHierarchyId($product_id, $hid);
            }
        }
        ...
        if (is_array($fields))
        {
            foreach ($fields as $one)
            {
                ...
                {
                    continue; // we don't import these.
                }
                ...
                {
                    // it's an image...
                    if (! $this->_getPolicyValue('handle_images') || empty($val) || empty($this->_imageHandler)
                        || ! is_object($this->_imageHandler))
                    {
        ...
        {
            $query = 'DELETE FROM ' . cms_db_prefix() . 'module_ec_prodmgr_product_categories WHERE product_id = ?';
            $db->Execute($query, array($product_id));
        }
        $cats = $this->_getFieldNamesLike('#CAT');
        if (is_array($cats))
        {
            $iquery = 'INSERT INTO ' . cms_db_prefix() . "module_ec_prodmgr_product_categories
            ...
            foreach ($cats as $one)
            {
                ...
                if (! (int) $val && ! $this->_getPolicyValue('clearcategories'))
                {
                    if ($cid)
                    {
                        $dquery = 'DELETE FROM ' . cms_db_prefix() . 'module_ec_prodmgr_product_categories'
        ...
        {
            $query = 'DELETE FROM ' . cms_db_prefix() . 'module_ec_prodmgr_attribs WHERE product_id = ?';
        ...
        $iquery = 'INSERT INTO ' . cms_db_prefix() . 'module_ec_prodmgr_attribs (product_id, text, adjustment, sku,'
                    . ' qoh, iorder) VALUES (?,?,?,?,?,?)';
        $iorder = 0;
    ...
    private function _testNewProduct(&$message)
    {
        ...
        {
            $this->_setError('invalid field count (' . $this->_getFieldCount() . ' of expected '
                                . $this->_getDefCount() . ')');
            return FALSE;
        }
    ...
    public function handleLine($line)
    {
        ...
        if ($pos1 != FALSE)
        {
            ...
            elseif ($pos3 !== FALSE && $pos3 == $pos1 - 6)
            {
                // its an https link
            }
            elseif ($pos4 !== FALSE && $pos4 == $pos1 - 4)
            {
                // its an ftp link
            }
            else
        ...
        else
        {
            ...
            {
                $res = $this->_testNewProduct($messages); // FIXME: $messages undefined
                if ($res)
                {
