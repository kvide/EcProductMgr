...
if (isset($params['fldname']))
{
    ...
    $query = 'SELECT * FROM ' . cms_db_prefix() . 'module_ec_prodmgr_category_fields
...
elseif (isset($params['submit']))
{
    ...
    try
    {
        $upload_handler = function ($id, $catid, $oldvalue, $allowed)
        {
            ...
            elseif (! $res && $err == \xt_fileupload::NOFILE)
            {
                return '-1';
            }
            if ($oldvalue)
            {
                @unlink($destdir . '/' . $oldvalue);
            }
            return $res;
        };
        ...
        $fieldtype = \xt_utils::get_param($params, 'input_fieldtype', $fieldtype);
        $fieldname = trim(\xt_utils::get_param($params, 'input_fieldname', $fieldname));
        $fieldprompt = trim(\xt_utils::get_param($params, 'input_fieldprompt', $fieldprompt));

        // Name can be empty (we'll generate one)
        if (empty($fieldname))
        {
            $query = 'SELECT COUNT(category_id)+1 FROM ' . cms_db_prefix() . 'module_ec_prodmgr_category_fields'
                        . ' WHERE category_id = ?';
        ...
        switch ($fieldtype)
        {
            ...
            case 'file':
                ...
                break;
            default:
                $fieldvalue = \xt_utils::get_param($params, 'input_fieldvalue', $fieldvalue);
                break;
        }
        ...
        if (! $fieldvalue)
        {
            throw new \RuntimeException('error_fieldvalue_empty');
        }

        // Store the field definition
        if ($old_fieldname)
        {
            $query = 'UPDATE ' . cms_db_prefix() . 'module_ec_prodmgr_category_fields
        ...
        {
            $query = 'SELECT MAX(field_order) AS num FROM ' . cms_db_prefix() . 'module_ec_prodmgr_category_fields
                       WHERE category_id = ?';
            ...
            $query = 'INSERT INTO ' . cms_db_prefix() . 'module_ec_prodmgr_category_fields
...
{
    case 'textbox':
        $smarty->assign('input_fieldvalue', $this->CreateInputText($id, 'input_fieldvalue', $fieldvalue, 80, 255));
        break;

    case 'textarea':
        $smarty->assign('input_fieldvalue', $this->CreateTextArea(true, $id, $fieldvalue, 'input_fieldvalue'));
        break;
