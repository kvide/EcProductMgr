...
try
{
    ...
    $ms_rawdata = \xt_param::get_string($params, 'cd_multiselect');
    if (! $ms_rawdata)
    {
        throw new \LogicException('Missing Parameter data for admin_bulk_editquantity action');
    }
    ...
    if (! $stock_field)
    {
        throw new \LogicException('Could not find the name of the quantity field');
    }
    ...
    if ($rs->totalrows == 0)
    {
        throw new \LogicException('Could not retrieve the products selected');
    }
    $list = array();
    while (! $rs->EOF())
    {
    ...
    if (isset($params['submit']))
    {
        try
        {
            ...
            foreach ($params['cd_qoh'] as $sku => $qoh)
            {
                ...
                if (! isset($list_a[$sku]))
                {
                    continue; // ignore anything not explicitly known
                }
                $obj = $list_a[$sku];
                if ($obj->attribute)
                {
                    // it's an attribute sku
                    $sql = 'UPDATE ' . cms_db_prefix() . 'module_ec_prodmgr_attribs SET qoh = ? WHERE sku = ?'
                                . ' AND product_id = ?';
                ...
                {
                    // it's a product sku
                    $sql = 'UPDATE ' . cms_db_prefix() . 'module_ec_prodmgr_fieldvals SET value = ? WHERE product_id = ?'
                                . ' AND fielddef_id = ?';
                    $db->Execute($sql, array(
                        $qoh,
                        (int) $obj->product_id,
    ...
    $tpl->assign('formstart', $this->XTCreateFormStart($id, 'admin_bulk_editquantity', $returnid,
                    array('cd_multiselect' => $ms_rawdata)));
