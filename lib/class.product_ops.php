...
final class product_ops
{
    ...
    static public function get_product($product_id, $anystatus = FALSE)
    {
        ...
        if (! $anystatus && $product->status != $product::STATUS_PUBLISHED)
        {
            return;
        }
    ...
    {
        $db = cmsms()->GetDb();
        $query = 'SELECT id FROM ' . cms_db_prefix() . 'module_ec_prodmgr_categories WHERE name = ?';
    ...
    {
        $db = cmsms()->GetDb();
        $query = 'SELECT id FROM ' . cms_db_prefix() . 'module_ec_prodmgr WHERE id = ?';
    ...
    public static function is_valid_hierarchy($str)
    {
        ...
        $db = cmsms()->GetDb();
        $query = 'SELECT id FROM ' . cms_db_prefix() . 'module_ec_prodmgr_hierarchy WHERE long_name = ?';
    ...
    {
        $db = cmsms()->GetDb();
        $query = 'SELECT id FROM ' . cms_db_prefix() . 'module_ec_prodmgr WHERE sku = ?';
        $parms = array($sku);
        if (! empty($productid))
        {
            $query .= ' AND id != ?';
        ...
        if (! $productonly)
        {
            $query = 'SELECT id FROM ' . cms_db_prefix() . 'module_ec_prodmgr_attribs WHERE sku = ?';
            $parms = [$sku];
            if (! empty($productid))
            {
    ...
    static public function check_alias_used($alias, $productid = '')
    {
        ...
        $parms = array();
        $parms[] = $alias;
        $query = 'SELECT id FROM ' . cms_db_prefix() . 'module_ec_prodmgr WHERE alias = ?';
        if (! empty($productid))
        {
            $query .= 'AND id != ?';
            $parms[] = (int) $productid;
    ...
    public static function get_product_hierarchy_id($productid)
    {
        ...
        {
            $db = cmsms()->GetDb();
            $query = 'SELECT hierarchy_id FROM ' . cms_db_prefix() . 'module_ec_prodmgr_prodtohier
    ...
    {
        $xtecb = \cms_utils::get_module('EcommerceExt');
        if (class_exists('\EcommerceExt\ecomm'))
        {
            return ecomm::get_currency_symbol();
        }
        $mod = \cms_utils::get_module(\MOD_ECPRODUCTMGR);
    ...
    public static function get_currency_code()
    {
        ...
        if (class_exists('\EcommerceExt\ecomm'))
        {
            return ecomm::get_currency_code();
        }
        $mod = \cms_utils::get_module(\MOD_ECPRODUCTMGR);
