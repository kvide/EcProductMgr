...
final class product_utils
{
    ...
    static public function get_feu_ownerlist()
    {
        ...
        $out = ['' => $mod->Lang('none')];
        $qry = new \mams_user_query();
        $qry->add_and_opt(\mams_user_query_opt::MATCH_GROUPID, $gid);
        $qry->add_and_opt(\mams_user_query_OPT::MATCH_NOTEXPIRED);
        $qry->set_sortby(\mams_user_query::RESULT_SORTBY_USERNAME);
        $qry->set_sortorder(\mams_user_query::RESULT_SORTORDER_ASC);
        $rs = $qry->execute();
        while (! $rs->EOF)
        {
            $row = $rs->fields;
    ...
    static public function get_field_types($selectone = false)
    {
        ...
        if (is_object($cdmod))
        {
            $items[$mod->Lang('companydir_reference')] = 'companydir';
        }
    ...
    public static function get_displayable_fieldval($fieldname, $fieldval)
    {
        ...
        $fields = \xt_array::to_hash($fields, 'name');
            ...
            case 'dimensions':
                {
                    ...
                    {
                        $res = sprintf('%s: %d, %s: %d, %s: %d', $mod->Lang('abbr_length'), $res['length'],
                            $mod->Lang('abbr_width'), $res['width'], $mod->Lang('abbr_height'), $res['height']);
                    }
                    else
                    {
                        $res = $mod->Lang('none');
            ...
            case 'subscription':
                {
                    if (is_array($res) && $res['payperiod'] != - 1 && $res['delperiod'] != - 1)
                    {
                        ...
                        $res = sprintf('%s: %s, %s: %s, %s: %s', $mod->Lang('subscr_payperiod2'),
                            $fieldval['payperiod'], $mod->Lang('subscr_delperiod2'), $fieldval['delperiod'],
    ...
    static public function hierarchy_isparentof($a, $b)
    {
        ...
        while ($b > 0)
        {
            ...
            if (! $node)
            {
                throw new \LogicException('Could not find hierarchy item with id ' . $b);
            }
            if ($node['parent_id'] == $b)
            {
                return TRUE;
    ...
    static public function hierarchy_get_tree($parent_id = - 1, $showall = 0, $callback_fn = '')
    {
        ...
        if (! is_array($hierarchy_map) || count($hierarchy_map) == 0)
        {
            return FALSE;
        }
        ...
        {
            if ($onehier['parent_id'] != $parent_id)
            {
                continue;
            }
    ...
    public static function update_hierarchy_positions()
    {
        ...
        $query = "SELECT id, item_order, name FROM " . cms_db_prefix() . "module_ec_prodmgr_hierarchy";
        ...
        while ($dbresult && $row = $dbresult->FetchRow())
        {
            ...
            {
                $query = "SELECT id, item_order, name, parent_id FROM " . cms_db_prefix()
                            . "module_ec_prodmgr_hierarchy WHERE id = ?";
                $row2 = $db->GetRow($query, array(
                    $current_parent_id
                ));
                if ($row2)
                {
                    $current_hierarchy_position = str_pad($row2['item_order'], 5, '0', STR_PAD_LEFT)
                                                            . "." . $current_hierarchy_position;
            ...
            {
                $current_hierarchy_position = substr($current_hierarchy_position, 0,
                                                        strlen($current_hierarchy_position) - 1);
            }
            ...
            $query = "UPDATE " . cms_db_prefix()
                        . "module_ec_prodmgr_hierarchy SET hierarchy = ?, long_name = ? WHERE id = ?";
    ...
    {
        $query = 'UPDATE ' . cms_db_prefix()
                    . 'module_ec_prodmgr_hierarchy SET parent_id = ?, item_order = ? WHERE id = ?';
        $db = cmsms()->GetDb();
    ...
    public static function get_categories($by_name = FALSE)
    {
        ...
        {
            $db = cmsms()->GetDb();
            $query = 'SELECT * FROM ' . cms_db_prefix() . 'module_ec_prodmgr_categories ORDER BY name ASC';
    ...
    public static function get_full_categories($by_name = FALSE)
    {
        ...
        if (! isset(self::$_category_cache[$fid]->data))
        {
            ...
            $query = 'SELECT * FROM ' . cms_db_prefix()
                        . 'module_ec_prodmgr_category_fields ORDER BY category_id,field_order';
            ...
            foreach ($keys as $one)
            {
                ...
                for ($i = 0; $dbr && $i < count($dbr); $i ++)
                {
                    // $catid = $dbr[$i]['category_id'];
                    if ($cat->id == $dbr[$i]['category_id'])
                    {
                        $cat->data[] = $dbr[$i];
                    }
                }
    ...
    static public function get_fielddefs($admin = false, $public = true, $as_hash = FALSE)
    {
        if (! is_array(self::$_flddef_cache))
        {
            ...
            {
                $query = 'SELECT * FROM ' . cms_db_prefix()
                            . 'module_ec_prodmgr_fielddefs WHERE public > 0 ORDER BY item_order';
            }
            else
            {
                $query = 'SELECT * FROM '
                            . cms_db_prefix() . 'module_ec_prodmgr_fielddefs WHERE admin_only <= 0 ORDER BY item_order';
            }
    ...
    public static function get_product_upload_path($product_id = null)
    {
        ...
        if ($product_id > 0)
        {
            $path = cms_join_path($path, 'product_' . $product_id);
        }
    ...
    public static function get_category_upload_path($category_id = null)
    {
        ...
        if ($category_id > 0)
        {
            $path = cms_join_path($path, $category_id);
        }
    ...
    public static function product_to_ecomm_productinfo(DisplayableProduct $product)
    {
        $out = new ProductMgr\productinfo();
        $out->set_source('EcProductMgr');
        $out->set_type($out::TYPE_PRODUCT);
        if ($product->is_service)
        {
            $out->set_type($out::TYPE_SERVICE);
        }
        ...
        if (count($product->attribs))
        {
            ...
            {
                $opt = new ProductMgr\productinfo_option();
                $opt->id = $attrib->id;
        ...
        if (count($product->field_vals))
        {
            ...
            if (! empty($alldefs) && ! empty($product->field_vals))
            {
                foreach ($product->field_vals as $fid => $val)
                {
                    if (! isset($alldefs[$fid]))
                    {
                        continue;
                    }
                    $def = $alldefs[$fid];
                    ...
                    switch ($def->type)
                    {
                        ...
                        case 'subscription':
                            $subscr = new ProductMgr\productinfo_subscription();
    ...
    public static function get_template($params, $key, $typename)
    {
        ...
        audit('', 'EcProductMgr', 'No default template of type ' . $typename . ' found');
        return;
    }
