...
class hierarchy_ops
{
    ...
    public static function get_all_hierarchy_info($count = FALSE, $all = TRUE)
    {
        ...
        {
            $db = cmsms()->GetDb();
            $query = 'SELECT ph.*, count(pr.id) AS direct_count FROM ' . cms_db_prefix() . 'module_ec_prodmgr_hierarchy ph
            ...
            if (! $all)
            {
                $query .= ' AND pr.status = ' . $db->qstr('published');
            }
    ...
    public static function get_breadcrumb($id, $hier_id, $hierpage, $delim = ' &raquo; ')
    {
        ...
        foreach ($hierarchy_path as $one)
        {
            ...
            $link = $module->CreateLink($id, 'hierarchy', $hierpage, $info['name'], array(
                'parent' => $info['id']
            ));
            $tmp[] = $link;
        }
    ...
    {
        if (! function_exists('\EcProductMgr\__hiertree_cb'))
        {
            ...
            function __hiertree_cb(&$row)
            {
                ...
                if (is_object($mod))
                {
                    $row['edit_url'] = $mod->create_url($id, 'admin_edit_hierarchy_item', $returnid, array(
                        'hierarchy_id' => $row['id']
                    ));
                    $row['depth'] = count(explode('.', $row['hierarchy'])) - 1;
                    $row['edit_link'] = $mod->CreateImageLink($id, 'admin_edit_hierarchy_item', $returnid,
                        $mod->Lang('edit_hierarchy_item'), 'icons/system/edit.gif', array(
                            'hierarchy_id' => $row['id']
                    ));
                    $row['delete_link'] = $mod->CreateImageLink($id, 'admin_delete_hierarchy_item', $returnid,
                        $mod->Lang('delete_hierarchy_item'), 'icons/system/delete.gif', array(
                            'hierarchy_id' => $row['id']
