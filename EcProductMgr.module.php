...
define('EcommerceExt\PRODMGR_PREF_NEWSUMMARY_TEMPLATE', 'products_pref_newsummary_template');
define('EcommerceExt\PRODMGR_PREF_DFLTSUMMARY_TEMPLATE', 'products_pref_dfltsummary_template');
define('EcommerceExt\PRODMGR_PREF_NEWDETAIL_TEMPLATE', 'products_pref_newdetail_template');
define('EcommerceExt\PRODMGR_PREF_DFLTDETAIL_TEMPLATE', 'products_pref_dfltdetail_template');
define('EcommerceExt\PRODMGR_PREF_NEWCATEGORYLIST_TEMPLATE', 'products_pref_newcategorylist_template');
define('EcommerceExt\PRODMGR_PREF_DFLTCATEGORYLIST_TEMPLATE', 'products_pref_dfltcategorylist_template');
define('EcommerceExt\PRODMGR_PREF_NEWBYHIERARCHY_TEMPLATE', 'products_pref_newbyhierarchy_template');
define('EcommerceExt\PRODMGR_PREF_DFLTBYHIERARCHY_TEMPLATE', 'products_pref_dfltbyhierarchy_template');
define('EcommerceExt\PRODMGR_PREF_NEWSEARCH_TEMPLATE', 'products_pref_newsearch_template');
define('EcommerceExt\PRODMGR_PREF_DFLTSEARCH_TEMPLATE', 'products_pref_dfltsearch_template');

/* TODO: Change to extend EcommModule */
...
final class EcProductMgr extends \CMSMSExt\XTModule
{
    ...
    public function __construct()
    {
        ...
        try
        {
            ...
            // making this stuff into static routes would allow lazy loading the EcProductMgr module and save exactly one query.
            $db = CmsApp::get_instance()->GetDb();
            $qparms = array('');
            $query = 'SELECT id, url FROM ' . cms_db_prefix() . 'module_ec_prodmgr WHERE url != ?';
            if (! isset($CMS_ADMIN_PAGE))
            {
                // for non admin requests we only need to register routes to published products.
                $query .= ' AND status = ?';
                $qparms[] = 'published';
            }
        ...
        \CMSMS\HookManager::add_hook('MAMS::OnDeleteUser', function ($params)
        {
            $id = \xt_param::get_int($params, 'id');
            if ($id < 1)
            {
                return;
            }
    ...
    public function SetParameters()
    {
        ...
        $smarty->registerClass('product_smarty', '\\EcProductMgr\\product_ops');
        $smarty->register_function('product_smarty_getcategory', array($this, '_smarty_products_getcategory'));
        $smarty->register_function('product_smarty_gethierarchy', array($this, '_smarty_products_gethierarchy'));
        $smarty->register_function('product_smarty_hierarchy_breadcrumb', array($this, '_smarty_products_hierarchy_breadcrumb'));
        $smarty->register_function('product_smarty_hierarchy_parent', array($this, '_smarty_products_hierarchy_parent'));
    }
    ...
    {
        return ['\EcProductMgr\AdminSearch_sl'];
    }

    function GetHeaderHTML()
    {
        $txt = parent::GetHeaderHTML();
        $obj = \cms_utils::get_module('JQueryTools', '1.2.6');
        if (is_object($obj))
        {
            $tmpl = <<<EOT
...
EOT;
    ...
    public function InitializeFrontend()
    {
        ...
        // Friendly URL stuff
        $detailpage = (int) $this->GetPreference('detailpage', - 1);
        if ($detailpage < 1)
        {
        ...
        $str = '/^' . $this->GetPreference('urlprefix', '[Pp]roduct');
    ...
    public function GetProductStorage()
    {
        ...
        if (! $obj)
        {
            $obj = new \EcProductMgr\ProductStorage($this, \xt_utils::get_db());
        }
    ...
    function GetCategory($category_id, $full = false)
    {
        ...
        $query2 = 'SELECT * FROM ' . cms_db_prefix() . 'module_ec_prodmgr_category_fields WHERE category_id = ?';
        ...
        $onerow->value = false;
        $onerow->file_location = \EcProductMgr\product_utils::get_category_upload_url($onerow->id);
    ...
    {
        return EcProductMgr\product_ops::get_search_result($returnid, $productid, $attr);
    }
    ...
    public function GetSearchableText(EcProductMgr\Product $product)
    {
        ...
        $fielddefs = EcProductMgr\product_utils::get_fielddefs(true);
        $results = array();
        ...
        {
            $results[] = strip_tags($attrib->text);
            $results[] = $attrib->sku;
        }
    ...
    {
        return \EcProductMgr\product_utils::update_hierarchy_positions();
    }
    ...
    {
        return array_flip(\EcProductMgr\hierarchy_ops::build_hierarchy_list());
    }
    ...
    {
        $this->autoload('product_utils');
        if ($inline || ! \EcProductMgr\product_utils::can_do_pretty($action, $params))
        {
            return;
        }
        $prettyurl = null;
        ...
        switch ($action)
        {
            ...
            case 'default':
                if (isset($params['categoryid']))
                {
                    ...
                    $prettyurl = sprintf("%s/bycategory/%d/%d", $this->GetPreference('urlprefix', 'product'),
                ...
                elseif (isset($params['hierarchyid']))
                {
                    // if the hierarchy id parameter is set, use byhierarchy
                    $nochildren = \xt_param::get_bool($params, 'nochildren');
                    if ($nochildren)
                    {
                    ...
                    {
                        $prettyurl = sprintf("%s/byhierarchy/%d/%d", $this->GetPreference('urlprefix', 'product'),
                ...
                elseif (\xt_param::exists($params, 'page'))
                {
                ...
                else
                {
                    ...
                    $prettyurl = sprintf("%s/summary/%d", $this->GetPreference('urlprefix', 'product'), $returnid);
                }
                break;
            ...
            case 'categorylist':
                ...
                {
                    $prettyurl = sprintf("%s/viewcategory/%s/%s", $this->GetPreference('urlprefix', 'product'),
                                            (int) $params['categoryid'], $returnid);
                }
                break;

            case 'hierarchy':
                $nochildren = \xt_utils::to_bool(\xt_utils::get_param($params, 'nochildren', 0));
                if (! $nochildren)
                {
                    $prettyurl = sprintf('%s/hierarchy/%d', $this->GetPreference('urlprefix', 'product'),
                                            (int) $params['parent']);
                    ...
                    if ($this->GetPreference('prettyhierurls', 0))
                    {
                        ...
                        $info = \EcProductMgr\hierarchy_ops::get_hierarchy_info($params['parent']);
                        if (is_array($info))
                        {
    ...
    {
        return \EcProductMgr\hierarchy_ops::get_hierarchy_info($hierarchy_id);
    }

    // deprecated
    function GetHierarchyPath($hierarchy_id)
    {
        return \EcProductMgr\hierarchy_ops::get_hierarchy_path($hierarchy_id);
    }

    // deprecated
    function GetProductHierarchyPath($productid)
    {
        return \EcProductMgr\product_ops::get_product_hierarchy_path($productid);
    }
    ...
    function _smarty_products_gethierarchy($params, &$smarty)
    {
        ...
        $obj = \EcProductMgr\hierarchy_ops::get_hierarchy_info($id);
    ...
    function _smarty_products_hierarchy_breadcrumb($params, &$smarty)
    {
        ...
        $bc = \EcProductMgr\hierarchy_ops::get_breadcrumb('prod', $hierid, $returnid, $delim);
    ...
    function _smarty_products_hierarchy_parent($params, &$smarty)
    {
        ...
        $info = \EcProductMgr\hierarchy_ops::get_hierarchy_info($hid);
        if (! is_array($info))
        {
            return;
        }
    ...
    function get_product_info($product_id)
    {
        ...
        $out = EcProductMgr\product_utils::product_to_ecomm_productinfo($product);
    ...
    public function set_price_adjuster(\EcProductMgr\PriceAdjuster $adjuster)
    {
    ...
    public function get_adjusted_price(\EcProductMgr\Product $product, $value)
    {
        $value = (float) $value;
        if ($this->_price_adjuster)
        {
