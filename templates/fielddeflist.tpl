...
$(function(){
  ...
  $('#revert_order').click(function(){
      window.location.href = window.location.href+'&m1_xt_activetab=fielddefs';
  });
...
{foreach from=$items item=entry}
	{cycle values="row1,row2" assign='rowclass'}
		<tr data-id="{$entry->id}" class="{$rowclass}" onmouseover="this.className='{$rowclass}hover';" onmouseout="this.className='{$rowclass}';">
			...
			<td><a href="{$entry->edit_url}">{xtimage image='icons/system/edit.gif' alt=$mod->Lang('edit')}</a></td>
