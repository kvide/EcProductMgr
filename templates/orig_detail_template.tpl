...
<div class="row">
  ...
  <div class="col-sm-10">
    {product_smarty::get_currency_symbol()}{$entry->price|as_num:2}
  </div>
</div>
  ...
  <div class="row">
    ...
    <div class="col-sm-10">
      <span style="color: red;">{product_smarty::get_currency_symbol()}{$foo.discount|as_num:2} ({$foo.percentage|as_num:2}%)</span>
    </div>
  </div>
  {/if}
{/if}
...
<div class="row">
  ...
  <div class="col-sm-10">
    {$entry->weight|as_num:2} {product_smarty::get_weight_units()}
  </div>
</div>
{/if}
...
{if !empty($entry->fields)}
  <fieldset>
    ...
    {foreach $entry->fields as $field}
	  ...
	  <div class="col-sm-10">
            {$EcProductMgr->lang('type')}: {$field->type}<br/>
	    {if !is_array($field->value)}
              {$EcProductMgr->lang('value')}: {$field->value}<br/>
	    {/if}
...
{if isset($entry->categories)}
  <fieldset>
    ...
    <div class="row product_detail_category"><p>
      {mod_action_url categoryid=$category->id assign='category_view_url'}
      ...
      <div class="col-sm-10">
        ...
        {if isset($category->data) && count($category->data)}
          <div class="product_detail_category_fields">
            <strong>{$EcProductMgr->Lang('data')}</strong><br/>
...
{anchor anchor='product_name' text=$EcProductMgr->Lang('return_to_top') title=$EcProductMgr->Lang('return_to_top')}
