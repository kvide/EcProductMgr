...
<div class="c_full">
  <div class="grid_6" style="padding-bottom: 0.25em;">
    <ul class="option-menu-horiz">
      <li>
        <a href="{mod_action_url action=addproduct}">{xtimage image='icons/system/newobject.gif' alt=''} {$mod->Lang('addproduct')}</a>
      </li>
      <li>
        <a href="{mod_action_url action=importproducts}">{xtimage image='icons/system/import.gif' alt=''} {$mod->Lang('import_from_csv')}</a>
      </li>
      {if $itemcount gt 0}
        <li>
        <a href="{mod_action_url action=exportcsv}">{xtimage image='icons/system/export.gif' alt=''} {$mod->Lang('export_to_csv')}</a>
        </li>
      {/if}
      <li><a id="filterbox">{xtimage image='icons/system/view.gif' alt=''} {$mod->Lang('filter')} {if $filterinuse}<em style="color: blue;">({$mod->Lang('filterinuse')})</em>{/if}</a></li>
    </ul>
  </div>
  ...
  <table cellspacing="0" class="pagetable">
	...
	<td>{if $entry.status != 'published'}
        ...
        {if !empty($filter.custom_fields)}
          ...
          {/foreach}
        {/if}
	<td><a href="{cms_action_url action=admin_edit_attribs prodid=$entry.id}">{xtimage image='table_relationship.png' alt=$mod->Lang('edit_options')}</a></td>
	<td><a href="{$entry.edit_url}" title="{$mod->Lang('edit')}">{admin_icon icon='edit.gif'}</a></td>
	<td><a href="{cms_action_url module=EcProductMgr action=admin_copyproduct compid=$entry.id}" title="{$mod->Lang('copy_product')}">{admin_icon icon='copy.gif'}</a></td>
	<td><a href="{cms_action_url action=deleteproduct compid=$entry.id}" title="{$mod->Lang('delete')}">{admin_icon icon='delete.gif'}</a></td>
