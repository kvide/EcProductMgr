...
if (isset($params['test']))
{
    ...
    $uh = new \xt_fileupload($id, $config['root_path'] . '/tmp/cache');
    ...
    if (! empty($error) && isset($importer) && is_object($importer))
    {
...
elseif (isset($params['go']))
{
    $this->Redirect($id, 'importcsv', $returnid, $params);
    $ext = $this->GetModuleInstance('CMSMSExt');
    $fn = $ext->GetModulePath() . '/lib/Progress2_Lite.php';
    include ($fn);

    if (! isset($_SESSION['products_import_data']))
    {
        die();
    }
...
$smarty->assign('formstart', $this->XTCreateFormStart($id, 'importproducts', $returnid, array(), false, 'post', 'multipart/form-data'));
